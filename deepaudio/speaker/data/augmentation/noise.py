...
class Noise:
    ...
    def __call__(self, waveform):
        ...
        return self.noise(waveform, sample_rate=self.configs.feature.sample_rate).squeeze(0)
...
class Reverb:
    ...
    def __call__(self, waveform):
        waveform = waveform.unsqueeze(0)
        return self.reverb(waveform, sample_rate=self.configs.feature.sample_rate).squeeze(0)
...
class NoiseReverb:
    ...
    def __call__(self, waveform):
        waveform = waveform.unsqueeze(0)
        return self.compose(waveform, sample_rate=self.configs.feature.sample_rate).squeeze(0)
